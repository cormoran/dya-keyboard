...
static uint8_t reg_read(uint8_t reg_address) {
    ...
    for (int8_t i = 7; i >= 0; i--) {
        ...
        digitalWrite(PIN_SDIO, reg_address & BIT(i));
        delayMicroseconds(1);
...
void _reg_write(uint8_t reg_address, uint8_t data) {
    ...
    for (int8_t i = 7; i >= 0; i--) {
        digitalWrite(PIN_SCLK, 0);
        digitalWrite(PIN_SDIO, data & BIT(i));
        delayMicroseconds(1);
        digitalWrite(PIN_SCLK, 1);
        delayMicroseconds(1);
    }
  ...
  uint8_t motion = reg_read(PMW3610_REG_MOTION);
    if (motion & BIT(7)) {
        ...
        if (true) {
          ...
          int16_t x =
                TOINT16(xl + ((xyh & 0xF0) << 4), 12);
          int16_t y =
                TOINT16(yl + ((xyh & 0x0F) << 8), 12);
